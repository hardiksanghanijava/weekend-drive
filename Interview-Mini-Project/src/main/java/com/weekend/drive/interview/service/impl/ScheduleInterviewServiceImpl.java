...
public class ScheduleInterviewServiceImpl implements  ScheduleInterviewService{
	...
	public List<ScheduleInterviewResponse> getAllScheduleInterviewDto(){
		...
		List<String> status = getCacheStatus();
		
		Cache cache = cacheManager.getCache("status-cache");
		logger.info("Cache : "+cache.get("#status"));
...
//		});
		...
		//Iterating entity to DTO
				for(ScheduleInterview interviews :  scheduleInterview) {
					scheduleInterviewDto.add(new ScheduleInterviewDto(interviews.getId(), interviews.getInterviewee().getId(),
							interviews.getInterviewer().getId(), interviews.getPositions().getId(),
							interviews.getRound().getId(), interviews.getTime(),status.get(count),
							interviews.isIsdeleted()));
					count++;
				}	
	...
	@Cacheable(value="status-cache", key="#status")
	public List<String> getCacheStatus() {
		List<ScheduleInterview> scheduleInterview =  scheduleinterviewRepository.findAll();
		
		List<String> statuses = new ArrayList<>();
		
		scheduleInterview.stream().forEach(interviews -> {
			statuses.add(interviews.getStatus());
		});
...
//		}
	...
	//Retrieve scheduled interview Dto by id
	public ScheduleInterviewDto getScheduleInterviewDtoById(int id) {
		Optional<ScheduleInterview> scheduleInterview = scheduleinterviewRepository.findById(id);
		logger.info("Find record by id : " + scheduleInterview);
		if(scheduleInterview.isEmpty())
			throw new IntervieweeNotFoundException("Scheduled Interview " + id + " Not Foud");
	
		ScheduleInterview interviews = scheduleInterview.get();
		
		//Creating DTO List
		ScheduleInterviewDto scheduleInterviewDto = new ScheduleInterviewDto(interviews.getId(), interviews.getInterviewee().getId(),
				interviews.getInterviewer().getId(), interviews.getPositions().getId(),
				interviews.getRound().getId(), interviews.getTime(),interviews.getStatus(),
				interviews.isIsdeleted());
